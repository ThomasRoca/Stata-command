...
/*==============================================================================================================================
! hdrdata v2.0 Thomas Roca, June 2015. 
This program connects to the Human Development Report API to load a dataset
This programm only uses web scrapping  
...
if missing("`country'") local mc=1
if !missing("`country'") local mc=0
if missing("`year'") local my=1
if !missing("`year'") local my=0

local countryList=subinstr("`countryList'", ","," ",.)  // country list without ","
local yearList=subinstr("`yearList'", ","," ",.)  // country list without ","

*======================================================================================
* For more flexibility, this program use web scrapping instead of insheetjson command
*======================================================================================
capture set maxvar 32767

*Scarp the data from the web 
local url "http://ec2-52-1-168-42.compute-1.amazonaws.com/version/1/indicator_id/`indicator_id'"
...
desc v1
drop if v1=="`indicator'"
local obs `r(N)'

***** Separate iso3, year and value, moving from 1 to 3 columns 
*Data are organized in a systematic way in v1 (ie. iso3, year and values being stacked)

*First generate 2 clones of v1
capture gen v2=v1
capture gen v3=v1

*erase value every 3 obs in v1
forvalues v = 1(3)`obs' {
capture replace v1="" in `v'
}
forvalues v = 2(3)`obs' {
capture replace v1="" in `v'
}

*erase value every 3 obs in v2, but this time starting from obs2 
forvalues v = 2(3)`obs' {
capture replace v2="" in `v'
}
forvalues v = 3(3)`obs' {
capture replace v2="" in `v'
}
*erase value every 3 obs in v3, but this time starting from obs3.. 
forvalues v = 1(3)`obs' {
capture replace v3="" in `v'
}
forvalues v = 3(3)`obs' {
capture replace v3="" in `v'
}

*Thus, we no longer have stacked informatin in v1,v2,v3 (value in v1, iso in v2 and year in v3)
* we still need to reorganise the data that still have an information every 3 observations.

*generate a variable containing the index (observation number)
gen n=_n

*get rid of country names still contained in the dataset after the observation that contains "country_names" in v2. So we drop observations after "country_names" 
levelsof n if v2=="country_name", local(n)
local N=_N
drop in `n'/`N'
drop n
**

*** Then we reorganise the datset, filling new variables iso and year according to v2 and v3
desc v2
local obs `r(N)'
capture gen iso=""
forval b=0/`obs' {
...
if `b'>0 { 
local a=`b'-1
if v2[`b']!="" | v2[`b']!="." replace iso=v2[`b'] in `b'
if v2[`b']=="" | v2[`b']=="." replace iso=iso[`a'] in `b'
	}
}
...
capture gen year=""
forval b=1/`obs' {
if `b'==1 replace year=v3[2] in 2
if `b'>0 { 
local a=`b'-1
if v3[`b']!="" | v3[`b']!="." replace year=v3[`b'] in `b'
if v3[`b']=="" | v3[`b']=="." replace year=year[`a'] in `b'
	}
}
*then drop v2 and v3 which are no longer usefull/
drop v2 v3


**** we now have complete series of year and iso, but still a value variable (v1) with no information every 3 obs. But we can drop empty obs ;)
drop if v1==""

*We now have a complete dataset that we can still polish by: coverting string when necessary, adding labels, and country names
...
*rename v1 using the indicator code
...
*present the data in the following order:
order cname iso3 year


/*We know have the complete dataset. 
NB. As it is faster to reply to webscrapping than insheetjson with loops, we prefered importing all serie (all years all countries)
then drop information according to user's request.*/


*------------------------------------------------------------------------------
* Follow user request regarding country and year to be displayed
*------------------------------------------------------------------------------

*User wants selected years
if `mc'==1 & `my'==0 {
capture gen keepy=0
foreach dy in `yearList' {
replace keepy=1 if year==`dy'
	}
capture keep if keepy==1
capture drop  keepy
}

* User wants selected country
if `mc'==0 & `my'==1 {
capture gen keepc=0
foreach dp in `countryList'{
replace keepc=1 if iso3=="`dp'"
	}	
capture keep if keepc==1
capture drop keepc
}

* User both wants selected years and country
if `mc'==0 & `my'==0 {
...
capture keep if keepc+keepy>1
capture drop keepc keepy
}
